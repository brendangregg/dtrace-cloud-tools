#!/usr/sbin/dtrace -s
/*
 * tcpretransport.d	Trace TCP rexmits to a specified port.
 *
 * USAGE: ./tcpretransport.d [port]
 *
 * Uses port 3306 (mysqld) by default.
 *
 * CDDL HEADER START
 *
...
dtrace:::BEGIN
{
	...
	port = $1 ? $1 : 3306;
	printf("Tracing TCP retransmits to remote port: %d\n\n", port);

	printf("%-20s %-17s %-15s %-15s %-5s\n", "TIME", "TCP_STATE",
	    "SRC", "DST", "LPORT");
}
